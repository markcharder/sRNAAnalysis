...
class smallRNAFastq(object):

	def __init__(self, object):
		self.fastq	= object

	def summariseFastq(self, prefix):
		with open(prefix + ".summary", 'w') as output:
			with open(self.fastq) as infile:
				for line in infile:
					if re.match("@", line):
						nextline	= next(infile)[:-1]
						string	= "\t".join([str(len(nextline)), nextline[0], nextline[-1]]) + "\n"
						output.write(string)
			infile.close()
		output.close()
	...
	def extractHairpins(self, reference):
		...
		for key in sorted(self.filteredclusters.iterkeys()):
			...
			if end >= len(fasta[self.filteredclusters[key][0]].seq):
				end	= len(fasta[self.filteredclusters[key][0]].seq)
